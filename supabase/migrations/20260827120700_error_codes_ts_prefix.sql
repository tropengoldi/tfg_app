-- PROJ-1 · Fehlercodes von PT### auf TS### umstellen
--
-- SQLSTATE der Form 'PT' + 3 Ziffern wird von PostgREST als HTTP-Statuscode
-- interpretiert (PT402 -> HTTP 402). PT001/PT002/PT004 sind keine gültigen
-- HTTP-Status -> der API-Gateway antwortet mit 'protocol error' und error.code
-- kommt als undefined an. Neue Codes: TS001..TS010 (Tasting-Fehler).
--   TS001 event_closed          TS006 position_already_poured
--   TS002 stale_position        TS007 last_round
--   TS003 whisky_limit_reached  TS008 order_invalid
--   TS004 not_authorized        TS009 participant_has_data
--   TS005 event_not_draft       TS010 wrong_status

begin;

-- Trigger-Funktion (aus 20260827120200_triggers.sql) neu mit TS001
...
-- Alle RPCs (aus 20260827120400_rpcs.sql) neu mit TS###
...
begin
  if not (public.is_admin() or public.is_event_host(p_event)) then
    raise exception 'Nur Gastgeber oder Admin.' using errcode = 'TS004';
  end if;
...
begin
  ...
  if v_next_pos > 10 then
    raise exception 'Mehr als 10 Whiskies pro Tasting sind nicht vorgesehen.' using errcode = 'TS008';
  end if;
...
begin
  ...
  if v_event is null then
    raise exception 'Whisky nicht gefunden.' using errcode = 'TS004';
  end if;

  select status into v_status from public.tasting_events where id = v_event for update;
  if v_status <> 'draft' then
    raise exception 'Nach dem Start lässt sich die Whisky-Liste nicht mehr ändern.' using errcode = 'TS005';
  end if;
  if not (v_bringer = v_uid or public.is_admin()) then
    raise exception 'Nur der Bringer oder der Admin darf den Whisky entfernen.' using errcode = 'TS004';
  end if;
...
begin
  if not (public.is_admin() or public.is_event_host(p_event)) then
    raise exception 'Nur Gastgeber oder Admin dürfen die Reihenfolge setzen.' using errcode = 'TS004';
  end if;
...
begin
  ...
  -- Proaktiv prüfen statt die Unique-Violation abzufangen (siehe 120600).
  if exists (
...
begin
  if not (public.is_admin() or public.is_event_host(p_event)) then
    raise exception 'Nur Gastgeber oder Admin dürfen die Runde weiterschalten.' using errcode = 'TS004';
  end if;
  select status, current_position into v_status, v_cur
    from public.tasting_events where id = p_event for update;
  if v_status is null then
...
begin
  if not (public.is_admin() or public.is_event_host(p_event)) then
    raise exception 'Nur Gastgeber oder Admin dürfen das Event abschließen.' using errcode = 'TS004';
  end if;
  select status into v_status from public.tasting_events where id = p_event for update;
  if v_status is null then
    raise exception 'Event nicht gefunden.' using errcode = 'TS004';
...
begin
  if not (public.is_admin() or public.is_event_host(p_event)) then
    raise exception 'Nur Gastgeber oder Admin dürfen den Fortschritt abfragen.' using errcode = 'TS004';
  end if;
